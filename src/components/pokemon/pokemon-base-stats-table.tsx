...
export function PokemonBaseStatsTable({ stats }: PokemonBaseStatsTableProps) {
  ...
  return (
    <div className="rounded-2xl border border-border/50 bg-background/30">
        ...
        <table className="w-full border-collapse text-sm">
          <thead>
            <tr className="border-b border-border/50 text-left text-[11px] uppercase tracking-wide text-muted-foreground">
              ...
              <th scope="col" className="min-w-[6.5rem] px-3 py-2 font-medium">
                Bar
              </th>
            </tr>
          </thead>
      ...
      <div className="mt-3 space-y-2 px-3 pb-3 md:hidden">
        {rows.map((row) => (
          <div key={`${row.key}-range`} className="flex items-center justify-between rounded-lg border border-border/40 px-2.5 py-1.5 text-xs text-muted-foreground">
            <span className="font-medium text-foreground">{row.label}</span>
            <span className="tabular-nums">
              Lv100 {row.minLevel100} - {row.maxLevel100}
            </span>
          </div>
        ))}
      </div>
